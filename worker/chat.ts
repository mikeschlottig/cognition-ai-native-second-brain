...
export class ChatHandler {
  ...
  private async generateToolResponse(
    ...
    const followUpCompletion = await this.client.chat.completions.create({
      model: this.model,
      messages: [
        { role: 'system', content: 'You are Cognition, a second-brain assistant. Respond naturally and clearly to the tool results using Markdown.' },
  ...
  private buildConversationMessages(userMessage: string, history: Message[]) {
    const cognitionSystemPrompt = [
      `You are **Cognition** — an AI-native "Second Brain" assistant embedded in a local-first markdown knowledge base.`,
      ``,
      `Your job is to help the user *think*, *connect ideas*, and *organize knowledge* (Obsidian-like workflows).`,
      ``,
      `Core behaviors:`,
      `- Prefer concise, high-signal answers. Use Markdown formatting for structure.`,
      `- When relevant, surface **conceptual connections** between notes and suggest **bidirectional links** (e.g., [[Note Title]]).`,
      `- Offer actionable next steps: suggested follow-up notes, outlines, tags, or questions.`,
      `- If context is provided (active note / related notes), treat it as the source of truth.`,
      `- When user asks for summaries, produce key takeaways + meaningful connections.`,
      `- When user asks for tags, produce markdown-friendly hashtags.`,
      ``,
      `Tone: calm, helpful, synthesis-focused. Avoid generic web-dev guidance unless explicitly asked.`,
    ].join('\n');
    return [
      {
        role: 'system' as const,
        content: cognitionSystemPrompt
      },
